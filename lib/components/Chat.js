...
class Chat {
  constructor(username, password, channel, obs) {
    ...
    this.commands = ['host', 'unhost', 'start', 'stop', 'switch', 'raid', 'bitrate', 'refresh', 'trigger', 'sourceinfo', 'obsinfo', 'public', 'mod', 'notify', 'autostop', 'rec', 'noalbs', 'fix', 'alias', 'restart'];
    this.aliases = {
      o: 'obsinfo',
      s: 'sourceinfo',
      b: 'bitrate',
  ...
  restart() {
    (0, _child_process.exec)('sudo systemctl restart nimble', (error, stdout, stderr) => {
      if (error) {
        console.log("error: ".concat(error.message));
        this.say((0, _stringTemplate.default)(this.locale.restart.error));
        return;
      }
      ...
      this.say((0, _stringTemplate.default)(this.locale.restart.success));
