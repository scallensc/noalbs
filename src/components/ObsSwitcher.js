      ...
      case 'nimble':
        try {
          ...
          const status = {
            srt: {
              live:
                srtdata.SrtReceivers?.[0]?.state == 'disconnected' ||
                isEmpty(srtdata.SrtReceivers)
                  ? false
                  : true,
              rtt:
                srtdata.SrtReceivers?.[0]?.state == 'disconnected' ||
          ...
          if (status.srt.live) {
            this.bitrate = status.srt.bandwidth;
            this.rtt = status.srt.rtt;
          } else if (status.rtmp.live) {
            this.bitrate = status.rtmp.bandwidth;
            this.rtt = status.rtmp.rtt;
          } else {
            this.bitrate = null;
            this.rtt = null;
          }
