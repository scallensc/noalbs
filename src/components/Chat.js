...
class Chat {
  ...
  parse(message) {
    ...
    if (!message[pos + 2] == '')
      parsedMessage.message = message.slice(3).join(' ').slice(1);
  ...
  rec(bool) {
    if (!bool) {
      this.say(
        `[REC] ${
          this.obsProps.heartbeat.recording
            ? this.locale.rec.started
            : this.locale.rec.stopped
        }`
      );
      return;
    }
  ...
  async switch(sceneName) {
    ...
    const res = search(sceneName, this.obsProps.scenes, {
  ...
  restart() {
    exec('sudo systemctl restart nimble', (error, stdout, stderr) => {
      ...
      this.say(format(this.locale.restart.success));
  ...
  bitrate() {
    if (this.obsProps.bitrate != null) {
      if (
        this.obsProps.rtt != null &&
        this.locale.bitrate.success_rtt != null
      ) {
        this.say(
  ...
  public(bool) {
    this.handleEnable(
      ...
      bool,
      this.locale.handleCommands.public
    );
  }
  ...
  mod(bool) {
    this.handleEnable(
      'enableModCommands',
      bool,
      this.locale.handleCommands.mod
    );
  }

  notify(bool) {
    this.handleEnable(
      'enableAutoSwitchNotification',
      bool,
      this.locale.handleCommands.notify
    );
  }
  ...
  autostop(bool) {
    this.handleEnable(
      ...
      bool,
      this.locale.handleCommands.autostop
    );
  }
  ...
  handleEnable(name, bool, response) {
    if (!bool) {
      this.say(
        `${response} ${
          config.twitchChat[name]
            ? this.locale.handleCommands.enabled
            : this.locale.handleCommands.disabled
        }`
      );
      return;
    }
  ...
  noalbs(a) {
    if (a === 'version')
      this.say(`Running NOALBS v${process.env.npm_package_version}`);
  }
        ...
        case 'nginx':
          try {
            const response = await fetch(
              `${site}/control/drop/subscriber?app=${application}&name=${key}`
            );
  ...
  getLanguage() {
    ...
    fs.readFile(
      `${__dirname}/../../locales/${this.language}.json`,
      'utf8',
      (err, data) => {
        if (err) {
